...
def createView(name, viewtype):  # 定义函数名
    ...
    CONTROL_less = codecs.open(path + name + '\\' + 'CONTROL.less.ftl', 'w', encoding='utf-8')
    CONTROL_less.write('')  # 写入内容信息
    CONTROL_less.close()

    CONTROL_html = codecs.open(path + name + '\\' + 'CONTROL.html.ftl', 'w', encoding='utf-8')
    CONTROL_html.write('<#ibizinclude>\n../@MACRO/HTML/DEFAULT.html.ftl\n</#ibizinclude>')  # 写入内容信息
    CONTROL_html.close()
